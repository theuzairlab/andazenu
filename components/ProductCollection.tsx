...
export default function ProductCollection({
  ...
  const sortProducts = (productsToSort: Product[], option: string): Product[] => {
    switch (option) {
      case 'Featured':
        // Assuming featured products might have a featured flag or priority
        return [...productsToSort].sort((a, b) =>
          (b as any).featured ? 1 : (a as any).featured ? -1 : 0
        );

      case 'Best selling':
        // Assuming we might add a sales count in the future
        return [...productsToSort].sort(
          (a, b) => (b as any).salesCount || 0 - ((a as any).salesCount || 0)
        );
      ...
      case 'Price, low to high':
        return [...productsToSort].sort(
          (a, b) =>
            parseFloat(a.salePrice.replace('Rs.', '').replace(',', '')) -
            parseFloat(b.salePrice.replace('Rs.', '').replace(',', ''))
        );
      ...
      case 'Date, old to new':
        // Assuming we might add a createdAt field
        return [...productsToSort].sort(
          (a, b) =>
  ...
  // Update the component to use sorting
            ...
            <div className="flex-1 p-6 flex flex-col justify-center">
              <div>
                <Link href={`/products/${product.id}`} className="">
                  <h3 className="font-medium text-xl mb-2 hover:text-blue-400 transition-colors">
                    {product.title}
                  </h3>
                </Link>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 text-black">
        ...
        <div className="mt-6">
          ...
          {viewMode === 'grid' ? (
            ...
            <div className="space-y-6">{sortedProducts.map(product => renderProduct(product))}</div>
          )}
        ...
        {totalResults && totalResults > 0 && (
          <div className="flex justify-between items-center mt-10 border-t border-gray-200 pt-6">
            <div className="text-sm text-gray-600">
              Showing {sortedProducts.length} of {totalResults} products
            </div>
